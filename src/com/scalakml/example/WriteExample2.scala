...
object WriteExample2 {
  def main(args: Array[String]): Unit = {
    ...
    val point = Point() withAddedCoordinate new Location(-0.126236, 51.500152)
    // create a Placemark with the point, a name and open
    val placemark = Placemark() withGeometry point withFeaturePart FeaturePart() withName("London2, UK") withOpen(true)
    // create a kml root object with the placemark
    val kml = Kml() withFeature placemark
    // write the kml to the output file
    new KmlPrintWriter("../kml-files/London2.kml").write(Option(kml), new PrettyPrinter(80, 3))
    println("\n....WriteExample2 done...")
  }
}